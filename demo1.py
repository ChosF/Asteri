...
def BlackScholes(r, S, K, T, sigma, tipo_num=0):
    ...
    if tipo_num == 0:  # Call
        precio = S * norm_cdf_approx(d1) - K * np.exp(-r * T) * norm_cdf_approx(d2)
    else:  # Put
        precio = K * np.exp(-r * T) * norm_cdf_approx(-d2) - S * norm_cdf_approx(-d1)
...
@njit(parallel=True)
def HeatMapMatrix_fast(Spot_Prices, Volatilities, Strike, Interest_Rate, T, tipo_num=0):
    rows, cols = len(Spot_Prices), len(Volatilities)
    M = np.zeros((rows, cols))
    
    for i in prange(rows):
        for j in range(cols):
            M[i,j] = BlackScholes(Interest_Rate, Spot_Prices[i], Strike, T, Volatilities[j], tipo_num)
...
call_price = BlackScholes_scipy(Risk_Free_Rate, Underlying_price, SelectedStrike, days_to_maturity / 365, volatility)
put_price = BlackScholes_scipy(Risk_Free_Rate, Underlying_price, SelectedStrike, days_to_maturity / 365, volatility, 'P')
...
def compute_heatmaps(spot_prices, volatilities, strike, rate, days):
    output_matrix_C = HeatMapMatrix(spot_prices, volatilities, strike, rate, days, type='C')
    output_matrix_P = HeatMapMatrix(spot_prices, volatilities, strike, rate, days, type='P')
    return output_matrix_C, output_matrix_P

output_matrix_C, output_matrix_P = compute_heatmaps(
    SpotPrices_space, Volatilities_space, SelectedStrike, Risk_Free_Rate, days_to_maturity)
...
with tab3: 
    ...
    step_map = {'Days': days_to_maturity, 'Hours': days_to_maturity * 24, 'Minutes': days_to_maturity * 24 * 60}
    ...
    def simulate(NS, days_to_maturity, s, volatility, Risk_Free_Rate):
        ...
        return simulate_paths_fast(NS, T, s, Underlying_price, volatility, Risk_Free_Rate)
    
    simulation_paths = simulate(NS, days_to_maturity, step, volatility, Risk_Free_Rate)
